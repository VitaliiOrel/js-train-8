...
function getUsersFromDatabase(array) {
  const arrayUsers = Array.from(array, function (el){
...
function filterByCondition(arr, condition) {
  let arr1 = arr.filter(condition)// Використовуємо метод `filter` для фільтрації масиву
  return arr1// Повертаємо відфільтрований масив
}
...
function checkArray(obj) {
  if (obj == null || obj == undefined) {return "Помилка: Вхідний об'єкт є null або undefined."}// Перевіряємо, чи є об'єкт не null і не undefined
  if (Array.isArray(obj)) {return true}// Використовуємо метод `Array.isArray` для перевірки, чи є об'єкт масивом
...
function createArray(...elements) {
  ...
  console.log("Повідомлення про помилку")// Якщо не передано жодного елементу, виводимо повідомлення про помилку
...
function getElementAtIndex(arr, index) {
  if (arr.length == 0) { return undefined}// Перевіряємо, чи масив не є порожнім
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  let arr3 = arr1.concat(arr2)// Об'єднуємо два масиви за допомогою методу `concat`
  let arr3Reverse = arr3.reverse()// Обертаємо отриманий об'єднаний масив за допомогою методу `reverse`
  return arr3Reverse// Повертаємо об'єднаний та обернутий масив
}
...
console.log(combineAndReverseArrays({}, [4, 5, 6])); //Виведе [ 6, 5, 4, 3, 2, 1 ]
...
function findElementIndexes(arr, element) {
  ...
  let a = arr.indexOf(element)// Знаходимо індекс першого знайденого елементу за допомогою методу `indexOf`
  let b = arr.lastIndexOf(element)// Знаходимо індекс останнього знайденого елементу за допомогою методу `lastIndexOf`
  let arrInx = []// Створюємо пустий масив індексів знайдених елементів
  if (a) {arrInx.push(a)}// Додаємо перший знайдений індекс, якщо елемент знайдено
  if (a !== b) {arrInx.push(b)}// Додаємо останній знайдений індекс, якщо він відрізняється від першого
  return arrInx// Повертаємо масив індексів
}
...
function copyAndSwapElements(arr, target, start, end) {
  if (target > arr.length || start > arr.length || end > arr.length || end < start) {
   return "Error: invalid indexes" // Перевіряємо, чи індекси належать межам масиву якщо ні виводимо рядок Неприпустимі індекси
  }
...
// Завдання:10
// В даному завданні ми створимо функцію, яка використовує метод `sort` для сортування масиву об'єктів 
// за допомогою певного ключа.
...
function sortByKey(arr, key) {
  arr.sort(function(a,b) {// Використовуємо метод `sort` передаємо в нього два аргументи a та b, для сортування масиву об'єктів за заданим ключем
...
/**
 * Функція `customEvery` перевіряє, чи всі елементи масиву задовольняють заданому умову за допомогою 
 * методу `every`.
...
function customEvery(arr, condition) {
  if (!Array.isArray(arr)) {return false}// Перевірка вхідних параметрів
  // якщо arr не масив повертаємо false
  if (!(typeof condition === 'function')) {return false}// якщо condition не function повертаємо false
  let x = arr.every(condition)// Використання методу `every` для перевірки умови для кожного елементу масиву
  return x// Повернення результату перевірки
}
...
/**
 * Функція `customFill` заповнює вказаний діапазон елементів масиву заданою значенням за 
 * допомогою методу `fill`.
...
function customFill(arr, value, start = 0, end = arr.length) {
  if (!Array.isArray(arr)) {return false}// Перевірка вхідних параметрів
  // якщо arr не масив повертаємо false
  if (start > arr.length || end > arr.length || end < start) {return false} // якщо start та end не входять в діапазон від 0 до довжини масиву arr повертаємо false
...
function customShift(arr) {
  ...
  if (arr.length === 0) {return undefined}// перевірка чи масив не пустий,якщо так повертаємо undefined
  let shiftedElement = arr.shift()// видаляємо перший елемент та записуємо його в змінну shiftedElement
